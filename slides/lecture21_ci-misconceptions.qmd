...
[Stat 141 \| Fall 2026 <br> Monday, Week 9]{.custom-subtitle}
...
- We typically estimate the SE (i) with the bootstrap, or (ii) with a formula (we'll see this in a few weeks)
...
Researchers are interested in the COVID-19 **reproduction rate** (the average number of individuals each infected person further infects). They sample 50 infected individuals and perform contract tracing.
...
## Quantiles and Percentiles {style="font-size:0.9em;"}
...
1\. $\color{darkred}{\text{Statistic} \pm 2*\text{SE}}$
...
2\. $\color{darkred}{\text{Percentile Method}}$
...
```{r fig.height=3, fig.width = 7, echo = F}
...
- Two ways to construct 95% confidence intervals *specifically*
    - **SE Method**: $\text{Statistic} \pm 2*\text{SE}$ (estimate $SE$ via bootstrap)
    - **Percentile Method**: take the 0.025 and 0.975 quantiles of the estimated sampling distribution (via bootstrap)

- If we want a confidence level that *isn't* 95%, we can easily adapt the percentile method by taking different quantiles from our bootstrap distribution.
    - We can actually generalize the SE method to other levels too, but we won't see that for a few weeks.
    
- We should always interpret confidence intervals carefully, and in context.
