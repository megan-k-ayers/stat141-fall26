...
-   Office hours update
-   First homework assignment due on Thursday 9/10
-   There won't be a quiz on this first homework
...
library(tidyverse)
qod <- read.csv("data/animal_people.csv")
qod %>% count(answer) %>%
  ggplot(aes(y = reorder(answer, n), x = n)) +
  geom_col() +
  ylab("") +
  theme_classic() + 
  ggtitle("9/2 QOD: Are you a dog person, cat person, both, or other?") +
...
-   Learn five standard graphs for numerical/quantitative data:
    ...
    -   **Barplot**: one categorical variable, one quantitative variable$^*$
    -   **Scatterplot** and **Linegraph**: two quantitative variables
...
**Q:** What does a row represent here?
...
-   There are other layers, such as `scale_---_---()` and `labs()`, but we will wait on those for a bit.

<br>

-   We are about to touch on many details of graphs - focus on recognizing this general pattern, expect to revisit these slides to refresh on the specifics
...
- **Q**: are histograms used for *quantitative* or *categorical* variables?
...
-   **Q**: Is this `fill` an `aes`thetic mapping? 
...
-   **Q**: What variable is mapped to `fill`? 
...
- Barplots show the distribution of quantitative variables within distinct levels defined by a *categorical* variable
    - Often via **numerical summarization** of a categorical variable (counts)
...
- **Q**: How can we tell a histogram from a barplot?
...
near_Reed <- near_Reed %>% filter(Genus %in% c("Acer", "Quercus", "Pinus", "Thuja", "Pseudotsuga"))
...
-   Fix over-plotting (using `alpha`)
-   Add a **plot title** and **axis labels** using `labs`
...
- Similar, but one might be preferred depending on context.
...
## Adjusting Labels: Rotate

```{r bar4}
#| code-line-numbers: "7-10"
biketown <- biketown %>% mutate(month = case_when(month == "7" ~ "July",
                                                  month == "8" ~ "August",
                                                  month == "9" ~ "September"))
ggplot(data = biketown, 
       mapping = aes(x = month)) +
...
::: fragment
[This linked webpage is a permitted internet resource for going to town on color customization](https://r-charts.com/colors/)
:::
...
- **Be warned**: Hours of my life have been lost to this, it can happen to you too...
...
ggplot(data = near_Reed,
  ...
  theme(legend.position = "bottom")   # Customize details individually
```
...
- This is the major concept to remember
    - Plug in `histogram`, `boxplot`, `bar`, `point`, or `line` for the `geom_---` 
    - **Aesthetic mappings** required in `aes()` depend on the **geom** (see previous slides)

- Can then add more layers to add detail/context
