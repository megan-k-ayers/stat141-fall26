...
-   I *highly* recommend doing the reading before Monday's class!
...
-   **Explaining** relationships between variables in a data set so that we can **infer** relationships in the population
    -   Ex. We observe that a sample of Reed students who sleep more have higher test scores, infer about the population **in general**
...
:::{.fragment}
\begin{align*}
  \widehat{y} &= \widehat{\beta_0} + \widehat{\beta_1} \times x_\text{HmRun} \\
   &= 330.59 + 17.67 \times x_\text{HmRun}
\end{align*}
::: 
...
\begin{align*}
  \widehat{y} &= \widehat{\beta_0} + \widehat{\beta_1} \times x
\end{align*}
...
<!-- -   We could give our model a value of $x_\text{HmRun} = 20.5$, and we'd get out a numerical prediction for `Salary`. How is this different from the previous examples? -->
...
-   These plot model **residuals** against the **fitted values**
    - Use `get_regression_points()` to grab residuals from your model and data (next slide)
-   We don't want to see any trends here 

--------------------------------------------------------------------

## Assessing conditions: Residual Plot Code
...
### Assessing conditions: Normality (of residuals) (QQ-Plot)
...
- Go back to the **Assessing conditions: Residual Plot Code** slide to review the code for residual plots
...
- Key functions:
    - `lm()` to fit a model
    - `get_regression_table()` to calculate coefficients from a model
    - `get_regression_points()` to grab residuals/fitted values from a model
    - `get_regression_summaries` to calculate $R^2$ from a model
    - `predict()` to make predictions for new $x$ values from a model
...
### 4 week feedback - pace

```{r echo=F}
x <- read.csv("data/Week4-feedback.csv")
names(x) <- c("pace", "workload", "labs", "hw", "quiz", "megan-OH", "ca-OH")
ggplot(x, aes(x = pace, fill = pace)) + geom_bar() + guides(fill = "none") +
...
### 4 week feedback (workload)

```{r echo=F}
x$workload <- factor(x$workload, levels = c("Not quite enough work",
                                            "Just the right amount of work",
                                            "A little too much work",
                                            "Way too much work"))
ggplot(x, aes(x = workload, fill = workload)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#BD7FF8", "#00BA38",  "#619CFF", "#F8766D")) +
  theme(axis.text = element_text(size=14))
```

------------------------------------------------------------------------

### 4 week feedback (labs and HW)
...
```{r echo=F}
x$labs <- factor(x$labs, levels = c("Very helpful", "Somewhat helpful",
                                        "Neutral", "Somewhat unhelpful"))
ggplot(x, aes(x = labs, fill = labs)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#00BA38","#BD7FF8",   "#619CFF", "#F8766D")) +
  theme(axis.text = element_text(size=14))
```
...
```{r echo=F}
x$hw <- factor(x$hw, levels = c("Very helpful", "Somewhat helpful",
                                        "Neutral", "Very unhelpful"))
ggplot(x, aes(x = hw, fill = hw)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#00BA38","#BD7FF8",   "#619CFF", "#F8766D")) +
  theme(axis.text = element_text(size=14))
```
...
### 4 week feedback (quiz)

```{r echo=F}
x$quiz <- factor(x$quiz, levels = c("Somewhat easy", "Neutral",
                                        "Somewhat challenging", "Very challenging"))
ggplot(x, aes(x = quiz, fill = quiz)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#00BA38","#BD7FF8",   "#619CFF", "#F8766D")) +
  theme(axis.text = element_text(size=14))
```
...
```{r echo=F}
x$`megan-OH` <- factor(x$`megan-OH`, levels = c("Very helpful", "Somewhat helpful",
                                        "Neutral", "Haven't attended"))
ggplot(x, aes(x = `megan-OH`, fill = `megan-OH`)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#00BA38","#BD7FF8", "gray40",  "#619CFF")) +
  theme(axis.text = element_text(size=14))
```
...
```{r echo=F}
x$`ca-OH` <- factor(x$`ca-OH`, levels = c("Very helpful", "Somewhat helpful",
                                        "Neutral", "Haven't attended",
                                        "Somewhat unhelpful"))
ggplot(x, aes(x = `ca-OH`, fill = `ca-OH`)) + geom_bar() + guides(fill = "none") +
  scale_fill_manual(values = c("#00BA38","#BD7FF8", "gray40",  "#619CFF", "#F8766D")) +
  theme(axis.text = element_text(size=14))
```
