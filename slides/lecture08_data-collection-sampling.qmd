...
[Data Collection and Sampling]{.custom-title style="font-size: 2.5em;"}
...
::: fragment
→ Talk it through with a course assistant, a fellow Stat 141 student, or Megan so together we can verbalize the process of going from Q to A.
:::
...
![](img/DAW.png){width="40%" fig-align="center"}
...
![](img/cloud_population.png){width="55%" fig-align="center"}

In a **census**, we have data on the entire population! 

Lets us calculate summary statistics about the whole population (we call these **parameters**).
...
We calculate **statistics** based on our **sample** and use these as **estimates** of the population **parameters** that we really want to know.
...
**Cluster Random Sampling**: "Clusters" are non-homogeneous. We take a simple random sample of the clusters, and use all (or some) observations in *those* clusters as the sample.
...
-   We often want to learn about an entire **population** (ex. all US voters) but only have access to a **sample** (ex. 1000 survey respondents)
-   We can use a sample to learn about the population **IF** it's **representative** of the population
-   **Sampling bias** occurs when systematic differences between our sample and the population lead us to make incorrect conclusions
-   Three ways to obtain a representative sample:
    - **Simple random sampling** (select cards from *one* hat)
    - **Cluster random sampling** (select *some* from a group of many *similar* hats, take their cards)
    - **Stratified random sampling** (select some cards from *every* hat in a group of very *different* hats)
