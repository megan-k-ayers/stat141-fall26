...
```{r}
filter(dogs, Dog_Name == "Rosie" & Dog_Breed == "Terrier")
```
...
To extract just one variable it can sometimes be easier to just use `$` if
we don't need to data wrangle

```{r}
#| code-line-numbers: "1"
just_dog_name <- dogs_top5$Dog_Name
head(just_dog_name)
```
