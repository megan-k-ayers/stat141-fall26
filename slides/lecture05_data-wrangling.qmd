...
- First quiz will be next week during lab
    - Based closely on HW 2 exercises
    - No live coding - but maybe explaining code
...
```{r echo=F}
x <- read.csv("data/cereal-soup.csv")
ggplot(data = count(x, ans), aes(x = fct_reorder(ans, n), y = n, fill = ans)) + 
  geom_col(color = NA) +
  scale_color_manual(values = c("idk" = "gray40", "no" = "darkred",
                                "yes" = "dodgerblue")) +
  xlab("Your responses") + guides(fill = "none") + 
  labs(title = "QOD 9/9: Is cereal a soup?") +
  theme_bw() +
...
dogs_top5 <- dogs %>%
  select("Dog_Name", "Dog_Breed", "Neighborhood") %>%
  mutate(Dog_Breed = case_when(Dog_Breed == "Mixed Breed" ~ "Mixed", TRUE ~ "Single")) %>%
...
::: {.column .fragment width="50%"}
→ In our Lab 2, we saw that `ggplot2` drops `NA` observations before creating a graph.
:::
:::::
...
```{r}
filter(dogs, Dog_Name == "Rosie" & Dog_Breed == "Schnoodle")
```
...
The pipe operator `%>%` is a handy way to string multiple commands together in `dplyr`. 
...
... we can write:
```{r eval = FALSE}
dogs %>% filter(Dog_Name != "Freddy") %>% head(3)
```
:::
:::

::: {.column width=50%}
-   Either method works! As you develop coding skills, consider:
    - Readability
...
Think of the pipe operator `%>%` as flowing our data frame through each command.
...
To extract variables, we can use the `dplyr` funtion `select()`

```{r}
#| code-line-numbers: "1"
dogs_top5 %>% select("Dog_Name", "Dog_Breed") %>%
  head()
```
...
To extract just one variable for a one-time use, it can be easier to just use `$`
...
```{r}
ce <- ce_raw %>% select("NEWID", "PRINEARN", "FINCBTAX", "BLS_URBN", "HIGH_EDU", "TOTEXPCQ", "IRAX")
dim(ce)
```
...
(*Reminder that `TOTEXPCQ` is total quarterly expenditures.*)
...
- (*Time check - possibly skip and review independently*)
...
::: {.fragment}
**In groups of 3-4, on paper or the board, write pseudocode to answer**: what states in the west have the highest average mobility rate across their selective private and selective public colleges? 
:::
...
colleges %>%
  filter(tier_name %in% c("Selective private", "Selective public"),
         region == 4) %>%
  group_by(state) %>%
...
-   Five fundamental wrangling verbs from the `dplyr` package:
    - `count()`: reduces the data set to counts of categorical variables
    - `filter()`: subsets the data frame to rows that meet logical conditions
    - `arrange()`: sorts the data frame
    - `mutate()`: creates new columns in the data frame (can be combined with arithmetic, `case_when()`, and more)
    - `summarize()`: reduces the data set to summary statistics
-   One `dplyr` action:
    - `group_by()`:
        - Tells `R`: perform the following operation separately for levels of a categorical variable
        - Mostly used in combination with `summarize()` (sometimes with `mutate()`)
- Use the pipe `%>%` between function calls to perform several data wrangling steps at once

------------------------------------------------------------------------

## Next Time

- Shifting gears into probability
