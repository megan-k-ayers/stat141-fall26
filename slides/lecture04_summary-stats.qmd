...
- [Data Lab at Reed!](https://www.reed.edu/data-at-reed/data_lab/)
    - Many resources including drop-in help, online `R` tutorials
- Math/Stats colloquium tomorrow! Come and hear about examples of stats research! 
...
-   Learn five standard graphs for **visualizing** data: histograms, boxplots, barplots, scatterplots, and linegraphs
:::
...
Compute in R across all values:
...
- **Q**: How will the medians compare? 
...
**Q:** Why was the NYT newsletter statement flawed? 

<!-- If we have a really left skewed distribution (a few *very* dumb dogs, many reasonably smart dogs), it's totally possible that the 0.66 quantile is above the mean! By "average" they meant "median"  -->

<!-- Rachel saying everyone else has an "above average" # of legs is another funny story to show this point but maybe won't have time. -->

:::

:::

![](img/dumb_cute_dogs.png){height=4in fig-align="center"}
...
-   Want a statistic that summarizes how much observations **deviate** from the mean
...
- Test out on 5 values: `5, 10, 20, 5, 25`
...
::::: columns
::: {.column width="55%"}
::: {.nonincremental .fragment style="font-size: 0.9em;"}
-   We found how much each observation deviates from the mean.
-   Computed the (nearly) average of the **squared** deviations ($s^2$).
-   But because observations are squared, units differ from original data.
:::
::: {.nonincremental .fragment style="font-size: 0.9em;"}
-   The square root of the sample variance is called the **sample standard deviation** $s$.

![](img/sample_var_latex.png){width="60%"}
...
- \*(*AKA a two-way table, often presented horizontally rather than all vertically*)
...
**Q:** What's different between these two tables?
...
-   We'll frequently **summarize** quantitative data by assessing a distribution's:
    1. Center
    2. Spread (aka variability)
    3. Shape
    
- If we want to assess these visually, we should use a boxplot or histogram    

- We quantify center and spread by calculating (or sometimes by eyeballing) **summary statistics** 
   1. The **mean** or **median** (`mean()`, `median()`) for center
   2. The **standard deviation**, **variance**, or **interquartile range** (`sd()`, `var()`, `IQR()`) for spread
...
-   The teaching team would love to see you in office hours! Check Moodle for the schedule.
