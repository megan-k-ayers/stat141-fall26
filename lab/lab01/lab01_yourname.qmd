...
### At the end of Lab 1, follow this submission process:
...
-   At some point in the semester (but especially now, when you're starting out), the previous step will result in an error message. Error messages show up in red in the Console, and your file cannot be rendered if there is an error. When this happens, take a deep breath, read the error message, try to find where it says the error is coming from, and try to fix it. If you're stuck:
  
    - Ask for help from your peers, on Slack, or in office hours.
    
    - If you are up against a deadline and need to submit what you have, comment out the code causing the error by putting # in front those lines of code.
