...
These next three exercises (EDA / Salary Regression Model / Assess Salary Regression Models) were moved here wholesale from hw05, unedited. They still read as full from-scratch exercises -- **rewrite them before this lab runs** to match the new pedagogical goal:

- `mlr_demo/mlr_demo.qmd` (now living in this lab05 folder as a standalone reference file, using the `house` dataset) is presented live at the start of lab (~15 min) and walks through the full EDA -> model -> LINE diagnostics -> adjusted R^2 workflow.
- These three exercises should become a **shortened** exercise where students *adapt/repurpose the demo's code* for the `SalaryGender` setting, rather than building everything from scratch or answering fully independent prose questions. Point them at `mlr_demo/mlr_demo.qmd` to copy/adapt from.
- The goal is intentional scaffolding: case study (watch) -> lab (adapt the code) -> hw (go deeper, independently, on `colleges`). Keep enough repetition that it reinforces the workflow, but cut what's now redundant with the demo (e.g., the full model-comparison-via-R^2 exercise probably overlaps too much with the demo's adjusted-R^2 section).
- No bridge/callback text is needed pointing students back to this lab from hw05 -- decided that's unnecessary hand-holding given how much repetition/lecture exposure they'll already have had.
