...
b. Write `R` code to *filter* to only the rows corresponding to China using the `filter()` function.
...
c. Using `R` in a new chunk below, *create a new column* using `mutate()` that calculates the case fatality rate (the proportion of cases that resulted in fatality) for each row. What do you notice when comparing rates between the two countries?
