...
**d.**  A recent study of $n = 20$ dogs investigated the effects of classical music on dog separation anxiety (measured on a 0-10 point scale) and found inconclusive results. Researchers were unable to reject the null hypothesis of zero effect. Researchers report that classical music reduced anxiety in the sample by an effect size of -2.5, and provide a 95% confidence interval of [-6.5, 1.5] and a p-value of $p = 0.34$ for $\alpha = 0.05$. **What are 2 distinct possible reasons for why the study could have failed to reject the null hypothesis, and how could researchers adjust a future study to distinguish between these 2 cases?** You can assume that the study was perfectly implemented - failure to reject the null was not due to any sampling or implementation error or accident. (Note: the p-value being too high is not a reason for failure, it's how we know it failed!)
