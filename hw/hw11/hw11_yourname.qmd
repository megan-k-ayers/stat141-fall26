...
**d.** Now that we're controlling for `gender` (compared to Lab 11, where we didn't), state the relevant hypotheses and interpret the results of the hypothesis test for whether `bty_avg` is a meaningful predictor of `score`. You should appeal to the provided p-value and state what it means in context.  [Text only for this question]
...
**e.** Does your answer from the previous part (2d) change what you think we should conclude about the relationship between `bty_avg` and `score`? 
...
**f.** Using the regression table from Exercise 2 part (a), answer the following:
